...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel(
      ...
      br(),
      column(12, h2(
        "First, a breakdown of the sample shown on the map."
      )),
      fluidRow(column(12,
                      div(
                        tableOutput("variableBreakdown")
                      )),
               column(12,
                      div(
      ...
      column(
        12,
        h2("Now, lets look at the neighborhoods and neighborhood groups.")
      ),
      
      column(6,
             div(
               plotOutput(outputId = "neighborhoodBarplot")
             )),
      column(6, div(
        plotOutput(outputId = "neighborhoodGroupBarplot")
      ),
      br(),
      div(
        plotOutput(outputId = "neighborhoodListingsMakeup")
      ))
    ),
    tabPanel(
      "Neighborhood Groups",
      ...
      column(
        ...
        offset = 2,
        fluidRow(p(
          "The code categorizes neighborhoods in Chicago into different groups based on their geographical locations. The neighborhoods are assigned to broader areas or sides of the city to facilitate analysis and interpretation. The groupings are as follows:"
        ))
      ),
      column(8,
             offset = 2,
             ...
             fluidRow(
               p(
                 " Albany Park, Avondale, Lake View, Lincoln Park, Lincoln Square, Logan Square, North Center, Rogers Park, Uptown, Edgewater, West Ridge, Loop, Near North Side, North Side"
               )
             )),
      ...
      column(8,
             ...
             fluidRow(
               p(
                 "Archer Heights, Ashburn, Beverly, Gage Park, Garfield Ridge, Hermosa, West Elsdon, West Englewood"
               )
             )),
      column(8,
             offset = 2,
             ...
             fluidRow(
               p(
                 "Armour Square, Bridgeport, Burnside, Calumet Heights, Chatham, Chicago Lawn, Clearing, Douglas, Grand Boulevard, Hyde Park, Kenwood, South Chicago, South Deering, South Lawndale, South Shore, Auburn Gresham, West Garfield Park, Near South Side, Avalon Park, New City, Woodlawn, Fuller Park, Englewood"
               )
             )),
      column(8,
             offset = 2,
             ...
             fluidRow(
               p(" Austin, East Garfield Park, Humboldt Park, West Town")
             )),
      column(8,
             offset = 2,
      ...
      column(8,
             ...
             fluidRow(
               p(
                 "Fuller Park, Englewood, Lower West Side, Mckinley Park, North Lawndale, Brighton Park"
               )
             )),
      column(8,
             offset = 2,
             ...
             fluidRow(
               p(
                 "All neighborhoods not explicitly listed in the above groups are categorized as “Other.” These are neighborhoods which lie far beyond the defined Chicagoland geography."
               )
             ))
    ...
    tabPanel(
      ...
      fluidRow(column(
        12,
        div("Chicago Listing Visualization: Explanation", class = "header")
      )),
      br(),
      fluidRow(
        column(
          ...
          offset = 2,
          p(
            "For our final project, we elected to work with the Airbnb data set from insideairbnb.com. This data provides information on over 5,000 different Airbnb listings in the Chicago Area"
          )
        ),
        ...
        column(
          ...
          p(
            "Together, we think the map and summary tab give a holistic view of the airbnb data and how distinct variables are distributed across Chicago and in the data set itself."
          ),
          br(),
          h4("Shiny App: Key Widgets and Explanations"),
          p(
            "Variable selection - We wanted to be able to look at any of the continuous variables and how their densities differ across Chicago"
          ),
          
          p(
            "Sample selection - Allowing the users to choose how many listings are on the map lets them see the effect that more listings may have on the variable they have chosen. With fewer listings, it may seem as though neighborhood prices are fairly equal, but as you increase the sample size, you can see the variation between neighborhood groups increase."
          ),
          
          p(
            "Listing markers - The density map gives a good idea of where the variable is concentrated, but we thought that adding the listing markers was helpful, especially when the sample size was small. It shows exactly which listings are being pulled to create the density map"
          ),
          
          p(
            "Neighborhood group tab - We thought this was necessary to show which neighborhoods fall into which groups
                  "
          ),
          br(),
          h4("Next Steps and Future Explorations"),
          p(
            "Possible future enhancements include incorporating more advanced analytics, additional map layers, and more user customization options. We think it would be very interesting to look at the densities of two variables concurrently. With this addition, the summary tab could develop to also show the correlation between the two variables selected."
          ),
        )
      )
    )
...
server <- function(input, output, session) {
  ...
  output$variableBreakdown <- renderTable({
    ...
    chicago_subset() %>%
      summarise(
        Variable = var_title,
        Total = n(),
        Mean = mean(var_summarise, na.rm = TRUE),
        Distinct = n_distinct(var_summarise),
        Minimum = min(var_summarise, na.rm = TRUE),
        Maximum = max(var_summarise, na.rm = TRUE)
      )
  })
  ...
  output$neighborhoodListingsMakeup <- renderPlot({
    chicago_subset() %>% ggplot(aes(x = 1, fill = neighbourhood_group)) +
      ...
      coord_polar(theta = "y") +
      labs(title = "Number of Listings in Each Neighborhood Group",
           x = NULL,
           y = NULL) +
      theme_minimal() +
      theme(
